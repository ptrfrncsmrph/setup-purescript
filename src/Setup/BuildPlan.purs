...
type BuildPlan = Array { tool :: Tool, version :: String }
...
resolve :: Json -> Tool -> ExceptT Error Effect (Maybe { tool :: Tool, version :: String })
resolve versionsContents tool = do
  ...
  case field of
    ...
    Just (Exact v) -> liftEffect do
      ...
      pure (pure { tool, version: Version.showVersion v })
    ...
    Just Latest
      ...
      | tool `elem` [ Psa, PursTidy ] -> liftEffect do 
        ...
        pure (pure { tool, version: "latest" })

      -- otherwise lookup the version from `versionsContents`
